...
io.sockets.on('connection', function(socket){
	...
	socket.emit('server-send-user', { danhsach : arrayUser});
	socket.on('client-register-user', function(data){
		if (arrayUser.indexOf(data)== -1) {
			...
			io.sockets.emit('server-send-chat', { chatComent : "Đăng ký thành công user: "+ data})
			...
			io.sockets.emit('server-send-chat', { chatComent : "Đã tồn tại user: "+ data})
			tontai = true;
		}
